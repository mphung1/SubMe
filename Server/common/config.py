...
if modelName == 'quartznet':
    ...
    from Jasper.model import JasperDecoderForCTC, JasperBlock, JasperEncoder
...
def encoder(conf):
    ...
    for blk in conf[modelName]['encoder']['blocks']:
        validate_and_fill(JasperBlock, blk, optional=['infilters'],
                          ignore_unk=['residual_dense'])

    return validate_and_fill(JasperEncoder, conf[modelName]['encoder'])


def decoder(conf, n_classes):
    decoder_kw = {'n_classes': n_classes, **conf[modelName]['decoder']}
