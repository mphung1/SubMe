...
def audio_from_file(file_path, offset=0, duration=0, trim=False, target_sr=16000):
    ...
    samples = torch.tensor(audio.samples, dtype=torch.float).to(device)
    num_samples = torch.tensor(samples.shape[0]).int().to(device)
    print(audio)
...
class AudioSegment(object):
    ...
    def __init__(self, filename, target_sr=16000, int_values=False, offset=0,
