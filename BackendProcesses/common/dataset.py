...
class FilelistDataset(Dataset):
    def __init__(self, filelist_fpath):
        if type(filelist_fpath) is not list:
            self.samples = [line.strip() for line in open(filelist_fpath, 'r')]
        else:
            self.samples = filelist_fpath
...
class LibriSpeechDevCleanDataset(FilelistDataset):
    def __init__(self, audio_fpath=[]):
        self.samples = getListOfFiles('./datasets/LibriSpeech/dev-clean/')
...
def getListOfFiles(dir_path):
    ...
    for entry in listOfFile:
        ...
        if os.path.isdir(fullPath):
            ...
            if os.path.splitext(fullPath)[1] == '.flac':
...
def get_data_loader(dataset, batch_size, multi_gpu=False, shuffle=True,
